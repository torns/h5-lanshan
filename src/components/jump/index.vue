<template>
  <div class="jump">
    <el-dialog title="设置跳转" width="30%" :visible.sync="show" :append-to-body="true">
      <!-- {{ jump }} -->
      <div class="tab"></div>
      ...
      <div class="mt20">
        <el-form :model="jump" ref="jumpForm" label-width="95px" class="demo-ruleForm">
          <template v-if="jump.type == 0">
            不跳转
          </template>
          <template v-if="jump.type == 1">
            <el-form-item label="网页地址" prop="pathData" :rules="{
                required: true,
                message: '不能为空',
                trigger: 'label',
              }">
          ...
          <template v-if="jump.type == 2">
            <el-select v-model="jump.pathData" placeholder="请选择页面">
              <el-option v-for="item in project.pages" :key="item.value" :label="item.name" :value="item.id">
              </el-option>
            </el-select>
          </template>
          <template v-if="jump.type == 3">
            ...
            <el-form-item label="小程序路径" prop="pathData" :rules="{
                required: true,
                message: '不能为空',
                trigger: 'label',
              }">
