<template>
  <div class="source-edit">
    <el-dialog title="API" width="80%" :show="show" :visible.sync="visible" :close-on-click-modal="false" :append-to-body="true" @close="$emit('update:show', false)">
      <div class="flex">
        <div class="flex-1 pr20">
          ...
          <el-form :model="form" ref="form" label-width="80px">
            ...
            <el-form-item label="URL" prop="url" :rules="{
              ...
              <el-input v-model="form.url" size="small" type="textarea" :rows="2"></el-input>
            </el-form-item>
            <el-form-item label="Headers" prop="headers" :rules="{
                required: true,
                message: '不能为空',
                trigger: 'blur',
              }">
              <el-input v-model="form.headers" size="small" type="textarea" :rows="1"></el-input>
            </el-form-item>
        ...
        <div class="flex-1">
          ...
          <div class="dialog-button mt10 mb15" @click="$refs['sourceCode'].getData()">
          ...
          <code-filter v-if="filterVisible" :show.sync="filterVisible" :option="form" @save="save"></code-filter>

          <!-- 最终返回结果 -->
          <div class="dialog-title">结果</div>
          <div class="dialog-info">注意：返回结果应满足组件的数据结构</div>
          <div class="dialog-button mt10 mb15" @click="$refs['resultCode'].getFiltersData(form.code)">
            <i class="iconfont icon-chakan"></i>
            <span>预览数据</span>
          </div>
          <div class="dialog-screnn">
...
export default {
  ...
  methods: {
    ...mapMutations(["addRemote", "editRemote"]),
    save(code) {
      this.form.code = code;
    },
    submit() {
      if (this.option.type == "add") {
        this.addRemote(this.form);
        this.visible = false;
   
      } else {
        this.editRemote(this.form);
        this.visible = false;
      }
    },
  },
